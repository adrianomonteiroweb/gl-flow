...
export const LossReasonModal = ({ open, onOpenChange, leadId, onConfirm, onCancel, onOfflineConfirm }: LossReasonModalProps) => {
  ...
  const getSelectedReasonLabel = (): string | null => {
    if (!selectedReasonId) {
      return null;
    }

    if (isFreeform) {
      return 'Outro (formato livre)';
    }

    return reasons.find(r => r.id === selectedReasonId)?.name ?? null;
  };

  const selectedReasonLabel = getSelectedReasonLabel();
  ...
  const fetchReasons = useCallback(async () => {
    ...
    if (reasonsResult.success) {
      setReasons(reasonsResult.data as LossReason[]);
    }
    ...
    <Dialog
      ...
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Selecione o motivo pelo qual este negócio foi perdido. Ao confirmar, o lead será movido automaticamente para a etapa{' '}
            <strong className="text-foreground">Fechado</strong> com status <strong className="text-foreground">Negociação Perdida</strong>.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 py-2">
          <div className="space-y-2">
            <Label>Motivo</Label>
            {isLoadingReasons ? (
              <div className="flex h-10 items-center text-sm text-muted-foreground">Carregando motivos...</div>
            ) : (
