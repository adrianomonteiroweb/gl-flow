...
interface MultiSelectProps extends React.ButtonHTMLAttributes<HTMLButtonElement>, VariantProps<typeof multiSelectVariants> {
  ...
  /** The default selected values when the component mounts. Used only when component is uncontrolled. */
  defaultValue?: string[];
...
export const MultiSelect = React.forwardRef<HTMLButtonElement, MultiSelectProps>(
  (
    ...
    // Initialize with defaultValue only in uncontrolled mode
    React.useEffect(() => {
      if (!isControlled && defaultValue && defaultValue.length > 0) {
    ...
    const toggleOption = (option: string) => {
      const newSelectedValues = currentValues.includes(option) ? currentValues.filter(value => value !== option) : [...currentValues, option];
                  ...
                  <CommandItem key="all" onSelect={toggleAll} className="cursor-pointer">
                    <div
                      className={cn(
                        ...
                        currentValues.length === options.length ? 'bg-primary text-primary-foreground' : 'opacity-50 [&_svg]:invisible'
                      )}>
