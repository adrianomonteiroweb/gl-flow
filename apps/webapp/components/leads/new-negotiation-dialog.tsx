...
import { ArrowLeft, Loader2, Search, UserPlus, WifiOff } from 'lucide-react';
...
export const NewNegotiationDialog = ({ trigger, initialVehicle, open: controlledOpen, onOpenChange: controlledOnOpenChange, onCreated }: NewNegotiationDialogProps) => {
  ...
  const goToVehicleStep = (client: Client) => {
  ...
  const handleConfirm = async () => {
    ...
    try {
      const result = await createNegotiationForClient({ client_id: selectedClient.id, vehicle_model_id: selectedVehicle.id });
      ...
      toast.success(`Negociação criada para ${selectedClient.name}.`);
  ...
  return (
    <>
      <Dialog open={open} onOpenChange={setOpen}>
        ...
        <DialogContent className={cn('gap-0', step === 'vehicle' ? 'sm:max-w-[720px]' : 'sm:max-w-[480px]')}>
          <DialogHeader>
          ...
          {step === 'client' ? (
              ...
              {is_online ? (
                <>
                  ...
                  <div className="min-h-[120px]">
                    {!query.trim() && !hasSearched && (
                      <p className="py-8 text-center text-sm text-muted-foreground">Digite para buscar um cliente existente.</p>
                    )}
                    ...
                    {showEmptyState && (
                      <div className="flex flex-col items-center gap-3 py-6">
                        ...
                        <Button type="button" variant="outline" size="sm" className="gap-2" onClick={() => setClientDialogOpen(true)}>
                ...
                <div className="flex flex-col items-center gap-4 py-6">
                  ...
                  <p className="text-center text-sm text-muted-foreground">
                    A busca de clientes e o catálogo não estão disponíveis offline. Crie um novo cliente para associar à negociação.
                  </p>
                  <Button type="button" variant="outline" className="gap-2" onClick={() => setClientDialogOpen(true)}>
                    <UserPlus className="h-4 w-4" />
                    Criar cliente
                  </Button>
                </div>
            ...
            <div className="mt-4 space-y-3">
              ...
              <DialogFooter className="mt-4 flex flex-row justify-between gap-2">
                <Button type="button" variant="ghost" className="gap-1.5" onClick={() => setStep('client')} disabled={isCreating}>
                  <ArrowLeft className="h-4 w-4" />
                  Voltar
                </Button>
                <div className="flex-1" />
                <Button type="button" onClick={handleConfirm} disabled={!selectedVehicle || isCreating} className="gap-2">
